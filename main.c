...
cy_rslt_t app_hw_init(void)
{
    ...
    result  = cyhal_adc_channel_init_diff(&adc_channels[4], &adc_obj, POT_ADC, CYHAL_ADC_VNEG, &channel_config);
    if(result != CY_RSLT_SUCCESS)
    {goto return_err;}

    /* Initialize a channel A5 and configure it to scan P10_5 in single ended mode. */
    result  = cyhal_adc_channel_init_diff(&adc_channels[5], &adc_obj, BAT_ADC, CYHAL_ADC_VNEG, &channel_config);
    if(result != CY_RSLT_SUCCESS)
    {goto return_err;}
...
static void adc_continuous_event_handler(void* arg, cyhal_adc_event_t event)
{
    ...
    if(0u != (event & CYHAL_ADC_EOS))
    {
        ...
        {
        	adc_data[i] = cyhal_adc_read_uv(&adc_channels[i])/1000;
        }
    }
}
